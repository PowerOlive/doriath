...
import (
	...
	secp "github.com/rensa-labs/doriath/electrumclient/internal/secp256k1/secp256k1-go2"
)
...
func pubkeyFromSeckey(seckey []byte) []byte {
	...
	if ret := secp.PubkeyIsValid(pubkey); ret != 1 {
		log.Panicf("ERROR: pubkey invald, ret=%v", ret)
		return nil
	}
...
new_seckey:
	...
	if ret := secp.PubkeyIsValid(pubkey); ret != 1 {
		log.Panicf("ERROR: Pubkey invalid, ret=%v", ret)
		goto new_seckey
	}
...
new_seckey:
	...
	if ret := VerifyPubkey(pubkey); ret != 1 {
		...
		log.Panicf("ERROR: pubkey is invalid, for deterministic. ret=%v", ret)
		goto new_seckey
	}
...
func Sign(msg []byte, seckey []byte) []byte {
	...
	if len(sig_bytes) != 64 {
		log.Fatalf("Invalid signature byte count: %v", len(sig_bytes))
	}
...
func RecoverPubkey(msg []byte, sig []byte) []byte {
	...
	if ret != 1 {
		log.Printf("RecoverPubkey: code %v", ret)
		return nil
	}
